# Copyright (C) 2015  metis_crypto Project Contributors - see LICENSE file
...
from .common import JWException, JWKeyNotFound
from .common import json_decode, json_encode
from .jwe import JWE
from .jwe import default_allowed_algs as jwe_algs
from .jwk import JWK, JWKSet
from .jws import JWS
from .jws import default_allowed_algs as jws_algs
...
class JWT:
    ...
    def __init__(self, header=None, claims=None, jwt=None, key=None,
        ...
        :param key: A (:class:`metis_crypto.jwk.JWK`) key to deserialize
         the token. A (:class:`metis_crypto.jwk.JWKSet`) can also be used.
    ...
    def make_signed_token(self, key):
        ...
        the claims as the payload. See (:class:`metis_crypto.jws.JWS`) for
        details on the exceptions that may be raised.

        :param key: A (:class:`metis_crypto.jwk.JWK`) key.
        """
    ...
    def make_encrypted_token(self, key):
        ...
        the claims as the plaintext. See (:class:`metis_crypto.jwe.JWE`) for
        details on the exceptions that may be raised.
    ...
    def validate(self, key):
        ...
        :param key: A (:class:`metis_crypto.jwk.JWK`) verification or
         decryption key, or a (:class:`metis_crypto.jwk.JWKSet`) that
    ...
    def serialize(self, compact=True):
        ...
        with the serialize() functions of :class:`metis_crypto.jws.JWS` and
        :class:`metis_crypto.jwe.JWE` so that these objects can all be used
